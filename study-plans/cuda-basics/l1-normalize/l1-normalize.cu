...
__global__ void l1_normalize_reduce(const float* __restrict__ input, float* __restrict__ l1_norm, int N) {
    int idx = blockIdx.x * blockDim.x + threadIdx.x;
    
    // 1. Grid-stride loop accumulation in registers
    float local_sum = 0.0f;
    for(int i = idx; i < N; i += blockDim.x * gridDim.x) {
        local_sum += fabsf(input[i]);
    }

    float sum = block_reduce_sum(local_sum);
    if (threadIdx.x == 0) {
        atomicAdd(l1_norm, sum);
    }
}


__global__ void l1_normalize_kernel(const float* input, float* output, float * l1_norm, int N) {
    int idx = blockIdx.x * blockDim.x + threadIdx.x;
...
extern "C" void solve(const float* input, float* output, int N) {
    ...
    cudaMemset(l1_norm, 0, sizeof(float));

    l1_normalize_reduce<<<blocks, threads>>>(input, l1_norm, N);
    l1_normalize_kernel<<<blocks, threads>>>(input, output, l1_norm, N);
